...
type Engine struct {
	...
	preamble                          string
	aggregateGeneratedFilesPerPackage bool
}

func NewEngine(pg *wdl.Program) *Engine {
	e := &Engine{prog: pg, preamble: "Code generated by github.com/worldiety/macro. DO NOT EDIT.", aggregateGeneratedFilesPerPackage: true}
...
func (e *Engine) Emit() error {
	if e.aggregateGeneratedFilesPerPackage {
		for _, pkg := range e.prog.Packages() {
			var tmp []*wdl.File
	...
	for _, pkg := range e.prog.Packages() {
		for _, file := range pkg.Files() {
			if file.Modified() {
				...
				slog.Info("emit source", "file", fname)
			}
		}
	}
